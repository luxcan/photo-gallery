...
/// start a new album after a six-hour gap - and simulating it over 9,544
/// real photographs showed that not one album would ever span more than a
...
/// </remarks>
public sealed class AlbumClustererTests
{
    ...
    public void ANightsSleepDoesNotEndATrip()
    {
        ...
        PhotoGroup group = Assert.Single(AlbumClusterer.Group(photos));
    ...
    public void ADayWithNoPhotographsEndsTheRun()
    {
        ...
        IReadOnlyList<PhotoGroup> groups = AlbumClusterer.Group(photos);
    ...
    public void ARunLongerThanTheCapIsOfferedAsItsDaysInstead()
    {
        ...
        Assert.All(groups, group => Assert.Equal(1, group.Days));
        Assert.All(groups, group => Assert.Equal(AlbumKind.Period, group.Kind));
    }
    ...
    {
        List<DatedPhoto> photos = [.. Enumerable
            .Range(0, AlbumClusterer.LongestRunDays)
            .SelectMany(day => Day(Noon.AddDays(day), count: 9))];

        PhotoGroup group = Assert.Single(AlbumClusterer.Group(photos));

        Assert.Equal(AlbumClusterer.LongestRunDays, group.Days);
    }
    ...
    public void AGroupHasToEarnItsPlace(int count, int minutesApart, bool expected)
    {
        ...
        Assert.Equal(expected, AlbumClusterer.Group(photos).Count == 1);
    }
    ...
    public void ADayAtHomeIsNotCalledATrip()
    {
        ...
        Assert.All(
            AlbumClusterer.Group(photos),
            group => Assert.NotEqual(AlbumKind.Trip, group.Kind));
    }
    ...
    public void AWeekendFarFromHomeIsCalledATrip()
    {
        ...
        PhotoGroup trip = Assert.Single(
            AlbumClusterer.Group(photos), group => group.Kind == AlbumKind.Trip);
    ...
    {
        Assert.Empty(AlbumClusterer.Group([]));
    }
