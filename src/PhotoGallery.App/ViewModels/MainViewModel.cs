...
public sealed partial class MainViewModel : ObservableObject
{
        ...
        IActivityLog activityLog)
    {
        ...
        [
            ...
            new ActivitySection(ActivitySection.PeopleKey, "People", "\uE716", true),
    ...
    /// <summary>
    /// Copies face model files the user already has into the library.
    /// </summary>
    /// <remarks>
    /// The weights are 182 MB and are not carried inside the executable yet, so
    /// a feature whose model is missing offers this rather than being a dead
    /// end. Each file is checked against its known digest before it is kept.
    /// </remarks>
    public async Task ImportFaceModelsAsync(string folder)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(folder);

        IsDetecting = true;
        OverlayTitle = "Installing the face model";
        OverlayTarget = folder;
        OverlayStatus = "checking...";
        OverlayIsIndeterminate = true;

        try
        {
            using IServiceScope scope = _scopeFactory.CreateScope();
            IModelStore models = scope.ServiceProvider.GetRequiredService<IModelStore>();

            foreach (ModelId id in new[] { ModelId.FaceDetection, ModelId.FaceRecognition })
            {
                ModelDescriptor descriptor = models.Describe(id);
                OverlayStatus = descriptor.FileName;

                string source = Path.Combine(folder, descriptor.FileName);
                ModelState state = await Task
                    .Run(() => models.ImportAsync(id, source), CancellationToken.None)
                    .ConfigureAwait(true);

                if (state != ModelState.Ready)
                {
                    People.Status = File.Exists(source)
                        ? $"{descriptor.FileName} is not the file this app expects."
                        : $"{descriptor.FileName} is not in that folder.";
                    return;
                }
            }

            People.Status = "The face model is installed. Finding faces can start.";
        }
        catch (Exception ex) when (ex is IOException or UnauthorizedAccessException)
        {
            Append($"could not install the face model: {ex.Message}");
            People.Status = ex.Message;
        }
        finally
        {
