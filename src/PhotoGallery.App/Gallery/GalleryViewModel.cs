...
public sealed partial class GalleryViewModel : ObservableObject
{
    ...
    private async Task AcceptSearchAsync()
    {
        ...
        {
            // Never the exception's own words. It names a file and a state
            // this app invented - "The ContentText file is Missing" - which is a
            // sentence for whoever wrote the encoder and for nobody else. The
            // line under the box already says what is not installed and where to
            // fix it, so all this has to report is what was searched instead.
            DiagnosticLog.Write("search fell back to names and places", ex);
            SearchNote = "Searched names and places only.";
