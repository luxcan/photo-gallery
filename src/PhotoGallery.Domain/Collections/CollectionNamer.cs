...
public static class CollectionNamer
{
    ...
    public static string Name(CollectionFacts facts)
    {
        ...
        if (facts.Places.Count > 0)
        {
            ...
            return facts.Kind == CollectionKind.Trip ? $"{where} Trip" : where;
        }
    ...
    private static string Span(CollectionFacts facts)
    {
        int days = facts.Days;
        if (days == 1)
        {
    ...
    /// </remarks>
    private static bool CoversAWeekend(CollectionFacts facts)
    {
        bool saturday = false;
        bool sunday = false;
