...
/// new collection after six hours with no photographs. Simulated over 9,544
/// dated photographs in a real library, that produced 180 collections and not
...
/// twenty-four hours one collection swallowed twenty-five days.
///
/// So a <em>session</em> is photographs no more than six hours apart, and a
/// <em>collection</em> is a run of sessions on consecutive days. A day with no
/// photographs ends the run. On the same library that produces 243 collections,
...
/// </remarks>
public static class CollectionClusterer
{
    ...
    /// <summary>Shorter than this and a single burst would become a collection.</summary>
    public static readonly TimeSpan ShortestSpan = TimeSpan.FromMinutes(90);

    /// <summary>
    /// How far from the usual place a collection has to be to be called a trip.
    /// </summary>
    /// <remarks>
    ...
    public static IReadOnlyList<PhotoGroup> Group(IReadOnlyList<DatedPhoto> photos)
    {
        ...
        foreach (List<DatedPhoto> run in Runs(Sessions(ordered)))
        {
            ...
            if (days > LongestRunDays)
            {
                ...
                {
                    Keep(groups, [.. day], home, CollectionKind.Period);
                }

                continue;
            }

            Keep(groups, run, home, days > 1 ? CollectionKind.Event : CollectionKind.Day);
        }
        ...
        CollectionKind kind)
    {
        ...
        groups.Add(new PhotoGroup(
            ...
            IsAway(photos, home) ? CollectionKind.Trip : kind));
    }
    ...
    /// nine in a real library, so most collections cannot answer this, and
