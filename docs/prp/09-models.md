...
**Status: ◐ Half built — and the half that exists is now carrying two features
that were both supposed to get the other half**
...
through `<name>.partial` so an interrupted copy leaves nothing usable. Faces offer
**Use model files I have…** where the feature is, rather than being a dead end.

What is not built: bundling weights in the executable, downloading fetched ones,
the Settings → Models screen, removal, and the licence acceptance.
...
- [ ] A feature with no model offers the install where the feature is.
- [ ] The download states its size before it starts and is resumable to the
      extent that stopping never leaves a file the app will use.
- [ ] A truncated or corrupt model is reported as damaged, not run.
- [ ] A model can be supplied from a local file with no network at all.
- [ ] The face weights' licence is shown once before first use.
- [ ] The app still starts, opens a library and browses with no models present.
...
| The URL each fetched model comes from, pinned to a release | ⬜ **still open, and it is what blocks `EnsureAsync`** |
